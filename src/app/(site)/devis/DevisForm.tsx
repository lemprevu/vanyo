  ...
  return (
    <div className="gradient-border rounded-3xl bg-ink-card/60 p-6 sm:p-9">
          ...
          {STEPS.map((s, i) => (
            <div key={s} className="flex flex-1 items-center">
              <div className="flex flex-col items-center">
                ...
                <span className={`mt-2 hidden text-xs sm:block ${i <= step ? "text-white/70" : "text-white/35"}`}>
                  {s}
                </span>
              </div>
              {i < STEPS.length - 1 && (
                <div className={`mx-2 h-0.5 flex-1 rounded ${i < step ? "bg-vanyo-500" : "bg-white/10"}`} />
              )}
            </div>
          ))}
        </div>
      ...
      <form onSubmit={onSubmit}>
          ...
          >
            ...
            <div className={step === 0 ? "block" : "hidden"}>
              <div className="grid gap-4 sm:grid-cols-2">
                <FieldGroup label="Prénom" required><Input name="prenom" required placeholder="Jean" /></FieldGroup>
                <FieldGroup label="Nom" required><Input name="nom" required placeholder="Dupont" /></FieldGroup>
                ...
                <FieldGroup label="Email" required><Input name="email" type="email" required placeholder="vous@email.com" /></FieldGroup>
        ...
        {status === "error" && (
          <p className="mt-5 rounded-lg border border-rose-500/30 bg-rose-500/10 px-4 py-2.5 text-sm text-rose-300">
            {error}
          </p>
        )}
        ...
        <div className="mt-8 flex items-center justify-between">
          <button
            ...
            onClick={prev}
            disabled={step === 0}
            className="btn-premium btn-ghost px-5 py-3 text-sm disabled:opacity-0"
          >
